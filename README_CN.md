...
- `/tpastorage json-to-sqlite` - 将 JSON 数据转换为 SQLite（仅 OP）
- `/tpastorage sqlite-to-json` - 将 SQLite 数据转换为 JSON（仅 OP）
...
rtp:
  ...
  # 随机传送最大范围（方块）
  maxRange: 2000
# 数据存储配置，可选值：json、sqlite；默认使用 sqlite
storage:
  backend: sqlite
```
...
物品注册 ID，数据同时兼容 JSON 和 SQLite；旧数据会继续使用默认床图标。

### Warp GUI 自定义图标与管理员权限

所有玩家都可以通过 `/warps` 打开 GUI，并左键 Warp 进行传送。只有管理员可以：

- 右键 Warp：删除公共传送点
- `Shift + 左键` Warp：打开全部原版物品图标选择器
- `Shift + 右键` Warp：快速恢复默认末影之眼图标

`/setwarp`、`/delwarp`、`/renamewarp` 和 `/tpastorage` 均需要管理员权限。
`/tpals` 只会向管理员显示这些管理员命令，普通玩家不会看到无法执行的命令。

## 存储后端与数据转换

模组支持 JSON 和 SQLite 两种运行时存储后端，默认使用 SQLite：

- `storage.backend: sqlite`：使用 `config/tpa/storage.db`（默认）
- `storage.backend: json`：使用 `config/tpa/storage.json`

已有配置明确设置为 `json` 时仍会继续使用 JSON；新安装、缺少
`storage` 配置或配置值非法时使用 SQLite。
...
转换不会删除源文件。建议转换前备份数据文件；转换完成后修改
`storage.backend`，然后重启服务器，使新的运行时存储后端生效。
...
- JSON 玩家数据：`config/tpa/storage.json`
