...
- Home 图标数据支持 JSON 与 SQLite，并兼容没有图标字段的旧数据
- 新增 Warp GUI 自定义图标，仅管理员可打开图标选择器或重置 Warp 图标
- 新增 SQLite 运行时存储后端，可通过 `storage.backend` 在 `json` 和 `sqlite` 之间选择
- 新增仅 OP 可用的 `/tpastorage json-to-sqlite` 命令，将 `storage.json` 转换为 `storage.db`
- 新增仅 OP 可用的 `/tpastorage sqlite-to-json` 命令，将 `storage.db` 转换为 `storage.json`
- 新增 SQLite JDBC 依赖，并将驱动打包到模组 JAR 中

### 修复
- 修复模组更新后外部语言文件缺少新翻译键，导致聊天和 GUI 显示原始键名的问题
- 统一管理员命令权限：普通玩家的 `/tpals` 不再显示 Warp 管理与存储转换命令
- 修复 `/tpals` 未向 OP 显示 JSON 与 SQLite 转换命令的问题
...
- SQLite 设为新安装和缺失存储配置时的默认后端
- 新增 `storage.backend` 合法性校验，非法值自动回退为 `sqlite`
- 对负数 `home.playerMaximum` 进行校验，并自动修正为 `0`
- 更新中英文 README，补充 SQLite 配置、数据文件及双向转换说明
