...
public class TpaCommand {
    ...
    private static void sendRequest(ServerPlayer initiator, ServerPlayer target, boolean here) {
        ...
        // 通知目标（带接受/拒绝按钮，用 sendSystemMessage 避免弹窗）
        target.sendSystemMessage(
                getTranslatedText("commands.teleport_commands.tpa.received", target,
                        ...
                        .append("\n")
                        .append(getTranslatedText("commands.teleport_commands.tpa.accept", target)
                                .withStyle(ChatFormatting.GREEN, ChatFormatting.BOLD)
                                .withStyle(s -> s.withClickEvent(new ClickEvent.RunCommand("/tpaaccept " + initiatorName))))
                        .append(" ")
                        .append(getTranslatedText("commands.teleport_commands.tpa.deny", target)
                                .withStyle(ChatFormatting.RED, ChatFormatting.BOLD)
                                .withStyle(s -> s.withClickEvent(new ClickEvent.RunCommand("/tpadeny " + initiatorName))))
        );
                    ...
                    tpa.SERVER.execute(() -> {
                        initiator.displayClientMessage(
                                getTranslatedText("commands.teleport_commands.tpa.expired", initiator,
                                        Component.literal(hereText)).withStyle(ChatFormatting.RED, ChatFormatting.BOLD), true);
                        target.displayClientMessage(
                                getTranslatedText("commands.teleport_commands.tpa.expired", target,
                                        Component.literal(hereText)).withStyle(ChatFormatting.WHITE), true);
                    });
                }
            }
        }, 30_000);
