...
public class TeleportDelayManager {
    ...
    private static void startTimers(ServerPlayer traveller, PendingTeleport pt, Runnable doTeleport) {
        ...
        pt.countdownTimer = new Timer("tpa-countdown-" + traveller.getUUID(), true);
        pt.countdownTimer.scheduleAtFixedRate(new TimerTask() {
            @Override public void run() {
                scheduleTickOnMainThread(traveller, pt, doTeleport);
            }
        }, 0, 1000);
        pt.particleTimer = new Timer("tpa-particles-" + traveller.getUUID(), true);
        pt.particleTimer.scheduleAtFixedRate(new TimerTask() {
            @Override public void run() {
                scheduleParticlesOnMainThread(traveller, pt);
            }
        }, 0, 50);
    }
    ...
    private static void tickCountdown(ServerPlayer traveller, PendingTeleport pt, Runnable doTeleport) {
        ...
        if (pending.get(traveller.getUUID()) != pt) {
            return;
        }

        // 玩家离线或已死亡，取消
        if (!traveller.isAlive() || traveller.hasDisconnected()
                || !pt.destination.isAlive()
                || pt.destination.hasDisconnected()) {
            TeleportDelayManager.cancel(traveller.getUUID());
            return;
        }
        ...
        if (pt.secondsLeft <= 0) {
            ...
            if (!pending.remove(traveller.getUUID(), pt)) {
                return;
            }
    ...
    private static void scheduleParticlesOnMainThread(
            ServerPlayer traveller, PendingTeleport pt) {
        tpa.SERVER.execute(() -> tickParticles(traveller, pt));
    }
    ...
    private static void tickParticles(ServerPlayer traveller, PendingTeleport pt) {
        ...
        if (pending.get(traveller.getUUID()) != pt
                || !traveller.isAlive() || traveller.hasDisconnected()) {
            return;
        }
    ...
    public static void cancelAll() {
        for (PendingTeleport pt : pending.values()) {
            if (pt.countdownTimer != null) pt.countdownTimer.cancel();
            if (pt.particleTimer != null) pt.particleTimer.cancel();
        }
        pending.clear();
